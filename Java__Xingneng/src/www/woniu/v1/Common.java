...
public class Common {
	...
	public String sendGet(String getUrl) {
		...
		try {
			...
			urlConnection.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/52.0.2743.116 Safari/537.36 Edge/15.15063");
...
//				body += line;
	...
	private void readCookie(HttpURLConnection urlConnection) {
		Map<String, java.util.List<String>> fields = urlConnection.getHeaderFields();
		java.util.List<String> cookies = fields.get("Set-Cookie");
		if (cookies.size() > 0) {
		// if (cookies != null && cookies.size() > 0) {
			for (int i=0; i<cookies.size(); i++) {
				String value = cookies.get(i).split(";")[0] + ";";
				// 去除重复内容后将获取到的内容拼接到cookieValue后，用于下次请求
				if (!this.cookieValue.contains(value))
					this.cookieValue += value;
			}
			
			//System.out.println(this.cookieValue);
			this.cookieValue= "";
		}
	}
